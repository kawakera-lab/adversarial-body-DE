...
class Agent:
    ...
    def save_weights2(self, iteration, state_rms): 
        torch.save({"current_policy_state_dict": self.current_policy.state_dict(),
                    ...
                    "state_rms_count": state_rms.count}, "./CheckPoint/CheckPoints"+self.env_name+"3/"+ self.env_name  +str(iteration)+ "_weights.pth") ##
    def save_weights(self, iteration, state_rms,time_name): #
        torch.save({"current_policy_state_dict": self.current_policy.state_dict(),
                    "critic_state_dict": self.critic.state_dict(),
                    "actor_optimizer_state_dict": self.actor_optimizer.state_dict(),
                    "critic_optimizer_state_dict": self.critic_optimizer.state_dict(),
                    ...
                    "state_rms_mean": state_rms.mean,
                    "state_rms_var": state_rms.var,
                    "state_rms_count": state_rms.count}, "./CheckPoint2/"+self.env_name+"_CP"+time_name+"/"+ self.env_name  +str(iteration)+ "_weights.pth") ##             
      
#"./CheckPoint/CheckPoints"+self.env_name +"/"+self.env_name  +str(iteration)+ "_weights.pth"

    def load_weights(self):
    
        if self.env_name == "Ant":
        	checkpoint = torch.load("./"+self.env_name+"200000_weights.pth")  # 200000(or 50000) normal train
